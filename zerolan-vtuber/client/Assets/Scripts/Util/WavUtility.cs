...
namespace Util
{
    ...
    public class WavUtility
    {
        ...
        {
            int x = sizeof(Int16);
            byte[] result = new byte[data.Length * x];
            ...
            while (i < data.Length)
            {
                var bytes = BitConverter.GetBytes(Convert.ToInt16(data[i] * maxValue));
                Buffer.BlockCopy(bytes, 0, result, i * x, x);
                ++i;
            }

            // Validate converted bytes
            Debug.AssertFormat(data.Length * x == result.Length,
                "Unexpected float[] to Int16 to byte[] size: {0} == {1}", data.Length * x, result.Length);
