...
from constants import TASK_TIMEOUT_SECONDS, EVENT_QUEUE_MAXSIZE
from event.event_data import BaseEvent


class Timer:
    """Lightweight timer that tracks elapsed time without spawning threads.

    The timeout_handler is called lazily on stop() if the deadline was exceeded,
    rather than using a background thread per task.
    """
    def __init__(self, timeout: float, timeout_handler: Callable):
    ...
    def stop(self):
        ...
        if self._timeout and self._timeout_handler:
...
class AsyncTaskExecutor:
    def __init__(self, maxsize: int = EVENT_QUEUE_MAXSIZE):
        self._async_tasks: Queue[AsyncCoro] = Queue(maxsize=maxsize)
    ...
    async def _async_event_loop(self):
        while True:
            ...
            if task:
                ...
                if len(self._submitted_tasks) > 100:
                    self._submitted_tasks = [t for t in self._submitted_tasks if not t.done()]

    @typechecked
    def add_async_task(self, func: AsyncCoro):
        if self._async_tasks.full():
            try:
                dropped = self._async_tasks.get_nowait()
                logger.warning(f"AsyncTaskExecutor queue full, dropped oldest task: {dropped.name}")
            except Exception:
                pass
...
class SyncTaskExecutor:
    def __init__(self, max_workers: int | None = None, maxsize: int = EVENT_QUEUE_MAXSIZE):
        self._sync_tasks: queue.Queue = queue.Queue(maxsize=maxsize)
    ...
    def stop(self):
        ...
        try:
            ...
            self._sync_tasks.put_nowait(None)
        except Exception:
            pass
        try:
    ...
    def add_sync_task(self, func: SyncFunc):
        if self._sync_tasks.full():
            try:
                dropped = self._sync_tasks.get_nowait()
                logger.warning(f"SyncTaskExecutor queue full, dropped oldest task: {dropped.name}")
            except Exception:
                pass
...
class TypedEventEmitter:
    ...
    async def stop(self):
        if self.sync_executor_thread:
            ...
            self.sync_executor_thread.request_stop()
    ...
    def _add_task(self, func: Callable, event_data: BaseEvent):
        if inspect.iscoroutinefunction(func):
            # Coroutine function
            async_coro = AsyncCoro(target=func, name=func.__name__, timeout=TASK_TIMEOUT_SECONDS)
            async_coro.set_args(event_data)
            self._async_executor.add_async_task(async_coro)
        else:
            # Sync function
            sync_func = SyncFunc(target=func, name=func.__name__, timeout=TASK_TIMEOUT_SECONDS)
    ...
    def on(self, event: str):
        """Register a persistent listener for the given event type."""
