...
def find_file(files: List[dict], question: str) -> str | None:
    ...
    result.to_messages()
    response = _get_model().invoke(result)
...
def sentiment_analyse(sentiments: List[str], text: str) -> str:
    ...
    matched = next((s for s in sentiments if s in response.content), None)
    if matched is not None:
        return matched
    defaults = {"Default", "Normal", "默认", "正常"}
    if defaults & set(response.content.split()):
        return sentiments[0]
    return sentiments[0]
...
def summary_history(history: List[Conversation]) -> AIMessage:
    ...
    text = '\n'.join(f"[{conversation.role}]\n{conversation.content}" for conversation in history)
    prompt_template = ChatPromptTemplate.from_messages(
        [("system", system_template), ("user", "{text}")]
    )
...
def sentiment_score(text: str) -> float:
    ...
    try:
        ...
        logger.debug(f"sentiment_score: failed to parse float from LLM response: {e}")
        return 0.0
...
def memory_score(text: str) -> float:
    ...
    try:
        ...
        logger.debug(f"memory_score: failed to parse float from LLM response: {e}")
        return 0.0
