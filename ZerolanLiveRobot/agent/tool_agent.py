...
class ToolAgent(LangChainAdaptedLLM):
    ...
    def __init__(self, config: LLMPipelineConfig):
        ...
        self._response_cache: dict[str, tuple[float, ChatResult]] = {}
        ...
        try:
            ...
            if isinstance(tool_call, dict):
                try:
                    ...
                    logger.debug(f"Tool call dict missing required fields: {e}")
                    return None
                ...
                for call in tool_call:
                    try:
                        ...
                        logger.debug(f"Skipping malformed tool call entry: {e}")
    ...
    def _generate(self, messages: list[BaseMessage], stop: Optional[list[str]] = None,
        ...
        cache_key = str([(m.type, m.content[:200]) for m in messages])
        now = time.monotonic()
        ...
        self._response_cache[cache_key] = (now, result)
        # Evict stale entries
        if len(self._response_cache) > 64:
            cutoff = now - self._CACHE_TTL
            self._response_cache = {k: v for k, v in self._response_cache.items() if v[0] > cutoff}
        return result
