...
class WindowsScreen(BaseScreen):
    ...
    def __init__(self):
        ...
        if not hasattr(pyautogui, "screenshot"):
            raise RuntimeError("pyautogui does not have screenshot capability")
    ...
    def capture_with_title(self, win_title: str, k: float | None = None) -> Tuple[Image, Path]:
        ...
        if len(win_list) == 0:
            raise ValueError(f'Window capture failed: Can not find {win_title}')
        w = win_list[0]
        # Activate the window
        w.activate()
        return self._capture(w, k)
