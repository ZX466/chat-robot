    ...
    def _load(self, audio_paths: List[str]):
        ...
        if len(self.tts_prompts) <= 0:
            if self._is_remote:
                ...
                raise Exception(msg)
            logger.warning(f"No eligible TTS prompts found in: {self._prompts_dir}. "
                           f"TTS prompt selection will be unavailable. "
                           f"This is fine if you are using MimoTTS or BaiduTTS.")
