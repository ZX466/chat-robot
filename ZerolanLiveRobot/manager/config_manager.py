...
class ConfigGeneratedError(SystemExit):
    """Raised when a new config file has been generated and the user needs to edit it before running."""
...
@typechecked
def set_default_config_path(path: Path):
...
def save_config(config: ZerolanLiveRobotConfig, path: Path | None = None):
    if config is None:
        raise ValueError("None cannot be saved to config file.")
    if path is None:
        path = get_default_config_path()
