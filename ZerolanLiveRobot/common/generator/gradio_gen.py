...
def _add_field_component(field_info: FieldInfo, field_name: str, field_val: Any) -> gr.Component:
    ...
    if field_type == str or (field_type == Union[str | None]):
        ...
        if not isinstance(field_val, list):
            raise TypeError(f"Expected list, got {type(field_val)}")
    ...
    def launch(self, share: bool = False):
        ...
        with self.blocks:
            ...
            with gr.Row():
                ...
                def on_click(*args):
                    if not (len(args) == len(self.input_comps) == len(self._field_setters)):
                        raise RuntimeError("Mismatch between args, input_comps, and field_setters lengths")
            ...
            for row in val:
                for col in row:
                    if not isinstance(col, str):
                        raise TypeError(f"Expected str, got {type(col)}")
                    res.append(col)
