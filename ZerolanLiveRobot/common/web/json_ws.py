...
from constants import MAX_MESSAGE_SIZE_BYTES
...
class JsonWsServer(ThreadRunnable):
    ...
    def _handle_json_recv(self, ws: Connection):
        ...
        try:
            while True:
                try:
                    message = ws.recv()
                    if isinstance(message, str) and len(message) > self.MAX_MESSAGE_SIZE:
                        logger.warning(f"Message too large ({len(message)} bytes), discarding from {ws.id}")
                        continue
    ...
    def send_json(self, data: Any):
        ...
        logger.info(f"[WS Send] Sending to {len(self._connections)} client(s): {msg[:200]}")
        for conn in self._connections:
            try:
                conn.send(msg)
                logger.info(f"[WS Send] Sent OK to {conn.id}")
            except Exception as e:
                logger.error(f"[WS Send] Failed to send to {conn.id}: {e}")
