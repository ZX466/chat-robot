...
class TwitchService(AsyncRunnable):
    ...
    def __init__(self, config: TwitchServiceConfig):
        ...
        """
        super().__init__()
        if is_blank(config.channel_id):
            raise ValueError("No channel_id provided.")
        if is_blank(config.app_id):
            raise ValueError("No app_id provided.")
        if is_blank(config.app_secret):
            raise ValueError("No app_secret provided.")
    ...
    async def init(self):
        ...
        async def on_ready(ready_event: EventData):
            ...
            if ready_event.chat.is_connected():
                ...
                msg = f"Failed to join Twitch channel: {self._target_channel}"
                emitter.emit(LiveStreamDisconnectedEvent(platform="twitch", reason=msg))
                logger.error(msg)
