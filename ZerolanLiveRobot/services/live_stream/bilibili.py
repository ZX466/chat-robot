...
class BilibiliService(AsyncRunnable):
    ...
    def __init__(self, config: BilibiliServiceConfig):
        super().__init__()
        if not config.room_id or config.room_id <= 0:
        ...
        self._monitor = LiveDanmaku(self._room_id, credential=credential)
    ...
    def _init(self):
        ...
        async def handle_disconnect():
            ...
            if self._retry_count >= self._max_retry:
                msg = (
                    "Failed to connect to Bilibili server after max retries. You can try:\n"
                    "1. Check your Internet connection.\n"
                    "2. Check your credential information in `config.yaml`.\n"
                    "3. Update the `bilibili-api-python` package to the latest version."
                )
                logger.error(msg)
                emitter.emit(LiveStreamDisconnectedEvent(platform="bilibili", reason=msg))
