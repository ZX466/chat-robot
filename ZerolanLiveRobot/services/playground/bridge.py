...
class PlaygroundBridge(ZerolanProtocolWsServer):
    ...
    def __init__(self, config: PlaygroundBridgeConfig):
        ...
        # Cache local IPs to avoid repeated calls
        self._cached_ipv6 = self.server_ipv6
        self._cached_ipv4 = self.server_ipv4
        # Model dir => File ID
        self.live2d_models = {}
    ...
    def _on_client_hello(self):
        ...
        server_hello = ServerHello(ws_port=self._config.port,
                                   res_port=self._config.res_server_port)
    ...
    def load_live2d_model(self, bot_id: str,
        ...
        if file_id is None:
            ...
            if len(self.live2d_models) > _LIVE2D_MODEL_CACHE_LIMIT:
                oldest_key = next(iter(self.live2d_models))
                del self.live2d_models[oldest_key]
        if file_id is None:
    ...
    def show_menu(self, menu_list: List[Arg_MenuItem], destroy_last: bool = True):
        items = [
            SelectionItem(
                ...
                img_id=register_file(item.img_path) if item.img_path is not None else None
            )
