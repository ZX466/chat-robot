...
class Live2DViewer(ThreadRunnable):
    ...
    def __init__(self, config: Live2DViewerConfig):
        ...
        if not os.path.exists(self._model_path):
            raise FileNotFoundError(f"The specified Live2D model file does not exist: {self._model_path}")
    ...
    def start(self):
        ...
        # Use a timer to process Qt events so stop() can call app.quit()
        self._quit_timer = QTimer()
        self._quit_timer.timeout.connect(lambda: None)  # keep event loop alive
    ...
    def _sync_lip_loop(self):
        while self._sync_lip_loop_flag:
            try:
                audio_path = self._audios.get(timeout=0.1)
                self._canvas.wavHandler.Start(str(audio_path))
            except Exception:
                pass  # Empty queue or timeout is expected
