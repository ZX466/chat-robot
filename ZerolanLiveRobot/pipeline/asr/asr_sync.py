...
from pipeline.asr.bigasr_asr import BigASRPipeline
...
class ASRSyncPipeline(CommonModelPipeline):
    ...
    def __init__(self, config: ASRPipelineConfig):
        ...
        elif config.model_id == ASRModelIdEnum.BigASR and config.bigasr_config is not None:
            bigasr = BigASRPipeline(config=config.bigasr_config)
            self.predict = bigasr.predict
            self.stream_predict = bigasr.stream_predict
    ...
    def stream_predict(self, query: ASRStreamQuery, chunk_size: int | None = None) -> Generator[
        ...
        files, data = self.parse_query(query)
        response = self._session.post(url=self.stream_predict_url, files=files, data=data, timeout=self._timeout)
        response.raise_for_status()
