...
DEFAULT_REQUEST_TIMEOUT = 60  # seconds, covers connect + read
...
class AbstractPipeline(ABC):
    ...
    def __init__(self, config: AbstractPipelineConfig):
        ...
        """
        if config is None:
            raise ValueError("Pipeline configuration should not be None.")
...
class CommonModelPipeline(PredictablePipeline):
    ...
    def predict(self, query: AbstractModelQuery) -> AbstractModelPrediction | None:
        ...
        response = self._session.post(url=self.predict_url, stream=False, json=query_dict, timeout=self._timeout)
    ...
    def stream_predict(self, query: AbstractModelQuery, chunk_size: int | None = None) -> Generator[
        ...
        """
        query_dict = self.parse_query(query)
        response = self._session.post(url=self.stream_predict_url, stream=True, json=query_dict, timeout=self._timeout)
        response.raise_for_status()
    ...
    def parse_prediction(self, response: Response) -> AbstractModelPrediction:
        ...
        if content is None:
            raise ValueError("The HTTP response body contains None content.")
    ...
    def parse_stream_prediction(self, chunk: str) -> AbstractModelPrediction:
        if chunk is None:
            raise ValueError("The HTTP response body contains None chunk.")
...
class AbstractImagePipeline(CommonModelPipeline):
    ...
    def _predict(self, query) -> Response:
        ...
        if isinstance(parsed_query, dict):
            response = self._session.post(url=self.predict_url, json=query.model_dump(), timeout=self._timeout)
        elif isinstance(parsed_query, tuple):
            files, data = parsed_query[0], parsed_query[1]
            response = self._session.post(url=self.predict_url, files=files, data=data, timeout=self._timeout)

        if response is None:
            raise ValueError("No response got, please check `parse_query`.")
    ...
    def parse_query(self, query: any) -> Tuple[dict, dict] | dict:
        ...
        if os.path.exists(query.img_path):
            ...
            with open(query.img_path, 'rb') as img:
