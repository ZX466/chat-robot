...
class BaseAsyncPipeline:
    """Base class for async HTTP pipelines using aiohttp.

    Manages a lazily-created ClientSession and provides cleanup via close().
    """
    ...
    @property
    def session(self):
        """Return the lazily-initialized aiohttp ClientSession."""
...
@typechecked
async def stream_generator(response: ClientResponse, chunk_size: int = -1) -> AsyncGenerator[bytes, None]:
    while True:
